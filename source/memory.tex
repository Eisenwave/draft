...
\pnum
\mandates
\tcode{T} is an implicit-lifetime type\iref{basic.types.general} and
not an incomplete type.
...
\range{(char*)p, (char*)p + (n * sizeof(T))} denotes a region of
allocated storage that is a subset of the region of
storage reachable through\iref{basic.compound} \tcode{p}.

\pnum
\effects
If \tcode{n > 0} is \tcode{true}, equivalent to:
...
otherwise, a pointer that compares equal to \tcode{p}\iref{expr.eq}.
\end{itemdescr}
